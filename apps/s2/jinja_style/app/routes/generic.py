...
@app.route("/")
def index():
    return render_template("pages/homepage.html", app_name=app_name, icono=icono)
...
def icono_main(id_icono: int):
    for item in icono:
        if item["id"] == id_icono:
            item_icono = item
    return render_template("pages/icono_main.html", app_name=app_name, item_icono=item_icono)
